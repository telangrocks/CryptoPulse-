...
export class CoinbaseExchange implements Exchange {
  ...
  async createOrder(order: OrderRequest): Promise<OrderResponse> {
    try {
      ...
      return {
        ...
        executedQty: parseFloat(response.filled_size || '0'),
        cummulativeQuoteQty: parseFloat(response.filled_size || '0') * parseFloat(response.price || '0'),
        timestamp: Date.now(),
  ...
  async getOpenOrders(symbol?: string): Promise<OrderResponse[]> {
    try {
      ...
      return response.map((order: any) => ({
        ...
        cummulativeQuoteQty: parseFloat(order.filled_size || '0') * parseFloat(order.price || '0'),
        timestamp: Date.now(),
  ...
  private generateSignature(queryString: string): string {
    // Generate HMAC-SHA256 signature for Coinbase Pro API
