...
export class ExchangeManager {
  ...
  async getBestTicker(symbol: string): Promise<{ exchange: string; ticker: Ticker }> {
    ...
    return tickers[0]; // For now, return the first available ticker
  }
  ...
  async validateOrder(order: OrderRequest): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!order.type || !['MARKET', 'LIMIT', 'STOP_LOSS', 'STOP_LOSS_LIMIT'].includes(order.type)) {
    ...
    if ((order.type === 'STOP_LOSS' || order.type === 'STOP_LOSS_LIMIT') && !order.stopPrice) {
